...
// Line 2's cost/cache/out area is one bracketed usage group. Parts inside the
// brackets join with " | " — the one place pipes are allowed.
func TestUsageGroup(t *testing.T) {
	tmp := t.TempDir()

	out := stripANSI(run(t, usageGroupPayload(tmp), 0))
	assertContains(t, "full usage group matches the target shape", out, "$23.12 [(+156 −23) | 99% | 528]")
...
func TestUsageGroupPartsDropCleanly(t *testing.T) {
	...
	assertContains(t, "no lines part without lines data", out, "$0.42 [90% | 10]")
	...
	assertContains(t, "no lines part when total_lines_removed is absent", outHalf, "$0.42 [90% | 10]")
