...
- **internal/statusline/** (`package statusline`) — the renderer and its tests:
  ...
  - **render.go** — `renderMain` (full assembly + exact newline structure), the context bar/gradient, rate-limit segment, cache/out, session cost, lines changed, effort/fast badges, worktree tag, and PR badge.
...
- **stdin JSON contract**: full field list at https://code.claude.com/docs/en/statusline. Conditionally-absent fields are guarded via pointer/nil checks (replicating jq's `//` defaults):
  ...
  - `fast_mode` is a plain boolean; absent and `false` both mean no `[fast]` badge. `thinking.enabled` is not decoded, on purpose: a `[thinking]` badge was removed because extended thinking is on for every one of this user's sessions, so it carried no signal. `cost.total_cost_usd` renders as a white `$N.NN`; `cost.total_lines_added`/`total_lines_removed` render as `(+N −M)` (mutedGreen/mutedRed, U+2212 minus); the lines part needs both fields and drops when either is absent. The cost part drops when `total_cost_usd` is absent.
