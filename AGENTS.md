...
make screenshot   # regenerate examples/statusline.svg (needs jq; fetches freeze from the module proxy on first run)
```

The README embeds `examples/statusline.svg`, generated by `make screenshot` → `scripts/screenshot.sh`: the example payload is piped through the binary and the ANSI output rendered to SVG with charmbracelet/freeze (version pinned in the script). A workflow (`.github/workflows/screenshot.yml`) re-runs it on pushes to `main` that touch the renderer or the screenshot pipeline (its `paths` filter is the authority) and commits the SVG when the bytes changed — so generation must stay deterministic. The script guarantees that by rewriting `workspace.current_dir` to a fixture repo with fixed contents, forcing `worktree.name` so the location segment shows a `[wt:...]` tag rather than the random mktemp path, dating the fixture commit in the future (the sync-age clamp then always renders `synced 0s ago`), and pinning `TZ`. Keep those properties when touching the script; a nondeterministic screenshot makes CI commit churn on every push.
